...
const DragDropComponent = {
    ...
    initializeContainer(container) {
        const sortable = new Sortable(container, {
            ...
            // Фильтр: не дублировать перетаскивание на кнопках
            filter: '.card-btn, button, input, select, textarea',
            ...
            // Проверка перед перемещением - блокировать чужие карточки
            onMove: (evt) => {
                const cardElement = evt.dragged;
                const isOwner = cardElement.dataset.isOwner === 'true';

                // Запретить перемещение чужих карточек
                if (!isOwner) {
                    console.warn('⛔ Нельзя перемещать чужие карточки');
                    return false; // Отменить перемещение
                }

                return true; // Разрешить перемещение
            },
